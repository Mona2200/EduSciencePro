...
namespace EduSciencePro.Data.Repos
{
   public class ConfirmationCodeRepository : IConfirmationCodeRepository
   {
      ...
      {
         var codes = await _db.Codes.FirstOrDefaultAsync(c => c.Email == email);
         return codes;
      }
      ...
      public async Task<string> Save(string email)
      {
         ...
         var tryCode = await GetCodeByEmail(email);
         if (tryCode == null)
         {
            var conformCode = new ConfirmationCode() { Code = code, Email = email };
      ...
      {
         var code = await GetCodeByEmail(email);
         if (code != null)
