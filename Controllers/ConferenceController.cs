...
namespace EduSciencePro.Controllers
{
    public class ConferenceController : Controller
    {
        ...
        public ConferenceController(IConferenceRepository conferences, IUserRepository users, IOrganizationRepository organizations, INotificationRepository notifications)
        {
            ...
            if (conference != null)
            {
                ...
                if (organization != null)
                {
                    var notification = new Notification()
                    {
                        ...
                        Content = $"<a href='/GetUser?userId={user.Id}'>{user.FirstName} {user.LastName} {user.MiddleName}</a> подал(а) заявку на участие в конференции <a href='/LookingConference?conferenceId={conference.Id}'>{conference.Title}</a>"
                    };
